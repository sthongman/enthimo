...
def run_roc_analysis(
    ...
    class_pair: Sequence | None = None,
) -> pd.DataFrame:
    """Load a CSV from any absolute/relative path and compute ROC metrics."""
    ...
    feature_cols = list(features) if features else None
    if not feature_cols:
    ...
    for pos_label, neg_label in iter_class_pairs(unique_labels, class_pair):
        subset = df[df[label].isin([pos_label, neg_label])]
        label_series = subset[label]
...
def main(argv: List[str] | None = None) -> int:
    ...
    results_df = run_roc_analysis(args.csv, args.label, args.features, args.classes)
